...
def decode_tf_cand_tuple(cand_tuple):
    depth = cand_tuple['layer_num']
    return depth, list(cand_tuple['mlp_ratio'][0:depth]), list(cand_tuple['num_heads'][0:depth]), list(cand_tuple['embed_dim'][0:depth])
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, device, model, model_without_ddp, choices, val_loader, test_loader, output_dir):
        ...
        self.tf_cands_file = args.resume_of_cands
        self.tf_all_res = []
        self.tf_rankings = {}
        self.af_ranking = {} #{'autoformer':[cands_id ranking]}
        self.af_all_res = []

    def save_af_ranking_checkpoint(self):
        info = {}
        info['af_all_res'] = self.af_all_res
        info['af_ranking'] = self.af_ranking
        ranking_checkpoint_path = os.path.join(self.output_dir, "af_ranking-{}cands.pth".format(len(self.af_all_res)))
        torch.save(info, ranking_checkpoint_path)
        print('save af_ranking to', ranking_checkpoint_path)
    ...
    def save_checkpoint(self):
        ...
        checkpoint_path = os.path.join(self.output_dir, "checkpoint-{}.pth".format(self.epoch))
    ...
    def getStandardRanking(self): #将TA-TAS里面算出来的cand在autoformer里计算排名
        tf_cand_info = torch.load(self.tf_cands_file)
        self.tf_all_res = tf_cand_info['all_res']
        self.tf_rankings = tf_cand_info['ranking']

        for res in self.tf_all_res:
            af_res = {} #{'id':at_value}
            af_res['id'] = res['id']
            depth, mlp_ratio, num_heads, embed_dim = decode_tf_cand_tuple(res['cand'])
            cand_config = {}
            cand_config['layer_num'] = depth
            cand_config['mlp_ratio'] = mlp_ratio
            cand_config['num_heads'] = num_heads
            cand_config['embed_dim'] = embed_dim  # python中,*输出重复列表,由于保存的时候维度本来就是够得，这里不需要乘      
            n_parameters = self.model_without_ddp.get_sampled_params_numel(cand_config)
            af_res['params'] = n_parameters / 10.**6

            # print("rank:", utils.get_rank(), res[cand], af_res['params'])
            print("rank:", utils.get_rank(), res['cand'])
            eval_stats = evaluate(self.val_loader, self.model, self.device, amp=self.args.amp, mode='retrain', retrain_config=cand_config)
            #test_stats = evaluate(self.test_loader, self.model, self.device, amp=self.args.amp, mode='retrain', retrain_config=sampled_config)
            #eval_stats = evaluate(self.test_loader, self.model, self.device, amp=self.args.amp, mode='retrain', retrain_config=cand_config)

            af_res['acc1'] = eval_stats['acc1']
            af_res['acc5'] = eval_stats['acc5']
            af_res['loss'] = eval_stats['loss']
            #af_res['test_acc'] = test_stats['acc1']
            self.af_all_res.append(af_res)

            if 'autoformer' not in self.af_ranking:
                id_rank = OrderedDict()
                id_rank[af_res['id']] = af_res['acc1']
                self.af_ranking['autoformer'] = id_rank
            else:
                id_rank = self.af_ranking['autoformer']
                id_rank[af_res['id']] = af_res['acc1']
                id_rank = OrderedDict(sorted(id_rank.items(), key=lambda t: t[1], reverse=True))
                self.af_ranking['autoformer'] = id_rank

        return

    def cal_cand(self,cand):
        ...
        test_stats = evaluate(self.test_loader, self.model, self.device, amp=self.args.amp, mode='retrain', retrain_config=sampled_config)
        print('eval_acc1:{}'.format(eval_stats['acc1']))
        print('test_acc1:{}'.format(test_stats['acc1']))
    ...
    def get_random(self, num):  # 获得足够数量的候选
        print('random select ........')
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            self.update_top_k(
                self.candidates, k=self.select_num, key=lambda x: self.vis_dict[x]['acc']) #根据准确度更新排名前k的cands
            self.update_top_k(
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='/home/zzy/AutoFormer/out_put',
    ...
    # CNN Proxy对比实验
    parser.add_argument('--resume-of-cands', default='', help='resume_cands from TF-TAS')
    return parser
...
def main(args):
    ...
    #计算输入的cands排名
    # searcher.getStandardRanking()
    # searcher.save_af_ranking_checkpoint()
    # print('af_ranking:')
    # print(searcher.af_ranking['autoformer'])

    #测试cand的准确率
    #20 epoch
    #cand = (14, 4, 3.5, 4, 3.5, 4, 3.5, 3.5, 3.5, 4, 4, 3.5, 3.5, 4, 4, 3, 4, 4, 4, 4, 3, 3, 4, 4, 3, 4, 4, 4, 3, 192)
    #50 epoch
    #cand = (12, 4, 4, 3.5, 3.5, 4, 3.5, 4, 3.5, 3.5, 4, 4, 3.5, 3, 3, 3, 3, 4, 3, 3, 4, 3, 3, 3, 4, 216)
    # 50 epoch第二名
    #cand = (12, 3.5, 3.5, 3.5, 3.5, 4, 3.5, 4, 3.5, 3.5, 4, 4, 3.5, 3, 4, 3, 3, 4, 3, 3, 4, 3, 4, 3, 4, 216)
    # Autoformer cand
    #cand = (13,3.5,3.5,3.0,3.5,3.0,3.0,4.0,4.0,3.5,4.0,3.5,4.0,3.5,3,3,3,3,3,3,3,3,3,3,4,3,3,192)
    # cand = (14, 4, 4, 4, 3.5, 3.5, 4, 3.5, 4, 4, 4, 4, 4, 4, 4, 3, 3, 3, 4, 3, 3, 3, 4, 4, 4, 4, 3, 4, 3, 192)
    # searcher.cal_cand(cand)

    print('total ranking time = {:.2f} hours'.format(
        (time.time() - t) / 3600))
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(["--data-path", "/data02/ImageNet2012",
                      "--gp", "--change_qk",
                      "--relative_position", "--dist-eval", 
                      "--cfg", "/home/zzy/AutoFormer/experiments/supernet/supernet-T.yaml",
                      "--resume", "/home/zzy/TF_TAS/CHECKPOINT/supernet-tiny.pth", 
                      "--min-param-limits", "5",
                      "--param-limits", "7", 
                      "--data-set", "EVO_IMNET",
                      "--output_dir","/home/zzy/TF_TAS/all_output/get_ken_output_M/SEED_0/cal_af",
                      "--resume-of-cands", "/home/zzy/TF_TAS/all_output/get_ken_output_M/SEED_0/cal_tf/ranking_checkpoint-1000cands.pth",
                      '--world_size','4'
                      ])

# nohup python3 evolution.py > /home/zzy/AutoFormer/all_output/5/output.log 2> /home/zzy/AutoFormer/all_output/5/error.log &
    if args.output_dir:
