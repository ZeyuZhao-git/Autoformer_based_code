...
def evaluate(data_loader, model, device, amp=True, choices=None, mode='super', retrain_config=None):
    ...
    #100,000张图片(1000类,每类100张)用来验证这个候选的准确度
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        acc1, acc5 = accuracy(output, target, topk=(1, 5)) #acc1是top1准确率

        batch_size = images.shape[0]
        metric_logger.update(loss=loss.item())
        metric_logger.meters['acc1'].update(acc1.item(), n=batch_size) #batch_size是128张图片
